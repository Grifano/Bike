...
$(document).ready(function () {
	$('.slider').slick({
		...
		centerMode: true, // поумолчанию false. Ставит гоавный слайд всегда по центру
...
$('.filter__item').click(function (event) {
	...
	if (i == 1) { // если class .filter__item имеет data-filter="1" - значит...
		$('.portfolio__column').show(); // ...показать колонки.
	} else { // иначе...
		$('.portfolio__column').hide(); // ...спрятать все колонки.
		$('.portfolio__column.f_' + i).show(); // но показать те которые имеют клас f_ + i
	}
